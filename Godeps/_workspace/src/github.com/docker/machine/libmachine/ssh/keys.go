...
import (
	...
	gossh "github.com/eris-ltd/eris-cli/Godeps/_workspace/src/golang.org/x/crypto/ssh"
)
...
func (kp *KeyPair) WriteToFile(privateKeyPath string, publicKeyPath string) error {
	...
	for _, v := range files {
		...
		fmt.Printf("file to create: %s\n", v.File)
		if err != nil {
			fmt.Printf("fuck create %v\n", err)
			return ErrUnableToWriteFile
		}

		if _, err := f.Write(v.Value); err != nil {
			fmt.Printf("fuck write %v\n", err)
			return ErrUnableToWriteFile
		}
